...
function createServers(port) {
        ...
        ws.on('message', raw => {
            try {
                ...
                if (msg.type === 'ping') {
                    ...
                    if (globalGame)
                        ws.send(JSON.stringify({ type: 'state', state: snapshot(globalGame) }));
                }
            }
    ...
    httpServer.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
            if (port === START_PORT)
                console.warn(`[gateway] Port ${port} in use.`);
